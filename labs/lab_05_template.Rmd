...
The p-value from the dispersion test is 0.000023, so we have strong evidence that $\phi > 1$ (i.e., that there is overdispersion in our data). 
...
The fitted regression coefficients are the same as in Question 6, because the quasi-Poisson fit only changes the estimated standard errors, not the parameter estimates. The estimated dispersion parameter $\widehat{\phi} = 1.77$, and the standard errors for the quasi-Poisson model are

$$SE_{quasiPoisson} = \sqrt{\widehat{\phi}} SE_{Poisson}$$
...
```{r}
library(MASS)
m_nb <- glm.nb(death ~ tmpd, data = chicago)
summary(m_nb)
```
...
You could use either negative binomial or quasi-Poisson here. I'll use the negative binomial model.

Note that there are a lot of NAs in the `pm25median` column, so I will just use the `pm10median` column for pollution.

```{r}
m_nb <- glm.nb(death ~ tmpd + pm10median, data = chicago)
summary(m_nb)
```

$\log(\widehat{\mu}_i) = 4.887 - 0.0028 \ Temperature_i + 0.00094 \ Pollution_i$

### Question 12

First, calculate a 95% confidence interval for $\beta_1$:

$$-0.0028 \pm 1.96 \cdot 0.000094 = (-0.0030, -0.0026)$$

Now exponentiate:

$$(e^{-0.0030}, e^{-0.0026}) = (0.9970, 0.9974)$$

So we are 95% confident that a temperature increase of 1 degree is associated with a decrease in the average number of deaths by a factor of between 0.997 and 0.9974, holding pollution constant.
